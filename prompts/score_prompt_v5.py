...
1. **数据与引用准确性**：
   - 答案中出现的所有个人数据（指标、数值、单位）必须与【个人数据】或【专家建议】或【对话历史】保持一致。
...
4. **课程引用**：
   - 课程名称，必须用尖括号<>引用，必须仅能与【课程库】中存在的内容完全匹配。
   - 课程名称不得捏造，也不能重复使用【对话历史】中的信息加以拼凑。
4. **睡眠专属核对**（若涉及）：
...
3. **NUM_COMPARE_ERROR** 【strict】
   ...
   - 仅说明当前状态（例如：正常/高于正常范围/低于正常范围/基本处于正常范围/明显高于基线等），严禁进行任何疾病诊断。
4. **ARITH_ERROR** 【strict】
   - 简单的数学计算错误（加减乘除、百分比、时间差、日期、统计值的计算等），或者【个人数据】中的数据选取出现错误。
   - 请在 reason 中写明你的验算过程。
