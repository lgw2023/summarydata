...
from classify_personal_data import *
...
if __name__ == "__main__":
    from _personal_data_class_test_data import test_SingleMetricDetailRecord, test_PeriodValueSingleSummaryRecord, test_PeriodTextSummaryRecord, test_PeriodValueCompareRecord, test_PeriodValuemMultiSummaryRecord, test_SingleDateValueSingleSummaryRecord, test_SingleDateTextSummaryRecord, test_NoTimestampTextSummaryRecord, test_NoDateValueSummaryRecord, test_SingleMetricStatsRecord, test_SingleDateValueMultiSummaryRecord, test_UnparsedRawPersonalData
    # from _personal_data_class_test_data_data_diff_sample import test_SingleMetricDetailRecord, test_PeriodValueSingleSummaryRecord, test_PeriodTextSummaryRecord, test_PeriodValueCompareRecord, test_PeriodValuemMultiSummaryRecord, test_SingleDateValueSingleSummaryRecord, test_SingleDateTextSummaryRecord, test_NoTimestampTextSummaryRecord, test_NoDateValueSummaryRecord, test_SingleMetricStatsRecord, test_SingleDateValueMultiSummaryRecord, test_UnparsedRawPersonalData
    # _self_test_single_metric_detail_record(test_SingleMetricDetailRecord)
    # _self_test_period_value_single_summary_record(test_PeriodValueSingleSummaryRecord)
    # _self_test_period_text_summary_record(test_PeriodTextSummaryRecord)
    # _self_test_period_value_compare_record(test_PeriodValueCompareRecord)
    # _self_test_period_value_multi_summary_record(test_PeriodValuemMultiSummaryRecord)
    # _self_test_single_date_value_single_summary_record(test_SingleDateValueSingleSummaryRecord)
    # _self_test_single_date_text_summary_record(test_SingleDateTextSummaryRecord)
    # _self_test_no_timestamp_text_summary_record(test_NoTimestampTextSummaryRecord)
    # _self_test_no_date_value_summary_record(test_NoDateValueSummaryRecord)
    # _self_test_single_metric_stats_record(test_SingleMetricStatsRecord)
    # _self_test_single_date_value_multi_summary_record(test_SingleDateValueMultiSummaryRecord)
    # _self_test_unparsed_raw_personal_data(test_UnparsedRawPersonalData)
    ...
    # 从 data_diff_sample.csv 读取 data 列；每行对应一个样本，最终得到“等长”的字符串列表
    _csv_path = Path(__file__).resolve().parents[1] / "data_diff_sample.csv"
    with _csv_path.open("r", encoding="utf-8", newline="") as _f:
        _reader = csv.DictReader(_f)
        test_aggregate_patterns_to_formatted_text = [
            (row.get("data") if row else "") or "" for row in _reader
        ]
